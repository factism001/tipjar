...
export default function TipSuccessPage({ searchParams }: { searchParams: { ref?: string; reference?: string; trxref?: string } }) {
  ...
  return (
    <div className="pt-8 text-center max-w-[640px] mx-auto px-4">
      ...
      <p className="mt-2 text-sm text-anon-gray">Your tip is being confirmed. You will be redirected shortly.</p>
      {ref && (
      ...
      <p className="mt-2 text-xs text-anon-gray">If Paystack shows success, your creator will see it in their dashboard in ~30s (webhook).</p>
      <div className="mt-6 flex flex-col gap-3">
        ...
        <a href={`https://tipjar-gray.vercel.app`} className="flex min-h-[44px] w-full items-center justify-center rounded-full border border-[#E5E7EB] text-sm font-semibold">Explore creators</a>
      </div>
    </div>
  );
}
